...
// Callers hold lock_; this takes none of its own, or GetCapabilities and
// UpdateConnectedUser would deadlock on a non-recursive mutex.
...
X_RESULT InputSystem::GetState(uint32_t user_index, X_INPUT_STATE* out_state) {
  ...
  // The host's own filter, which the recomp framework uses to hold back the
  // buttons of its guide chord and to replay a press the guide swallowed. It
  // belongs here rather than in GetStateForUI: this is the guest's view, and a
  // dialog reading the pad should see what the player is actually pressing.
  if (result == X_ERROR_SUCCESS && out_state && state_filter_) {
    state_filter_(user_index, *out_state);
  }
...
void InputSystem::RemoveUIInputBlocker() {
  // Runs on the UI thread while a guest thread may be in GetState, and
  // GetStateForUI walks the device tables without a lock of its own.
  std::lock_guard<std::mutex> guard(lock_);
