...
// Native renderer hook: lets a game draw its frames through the host GPU
// (rex/graphics/native_rhi.h) instead of the emulated Xenos output, falling
// back to emulation frame by frame. Ported from the Skate 3 recompilation's
// SDK (github.com/mchughalex/rexglue-skate3, BSD-3-Clause), with the
// game-specific pass selection moved behind SetNativeGuestOutputPassFilter.
...
// currently targeting `surface_pitch`-wide surfaces be suppressed? Shared by
// both command processors. Draw and resolve suppression must agree: executed
// passes need their resolves, suppressed passes leave garbage EDRAM that must
// never be copied out.
//
// Which passes a native renderer can do without is game knowledge (a pass
// whose output it samples from guest memory must keep running), so a game
// registers a filter. Without one, native_render_suppress_mode decides:
// 0 = framebuffer-sized passes only (pitch >= 1280), 1 = every pass.
using NativeGuestOutputPassFilter = bool (*)(uint32_t surface_pitch, void* user_data);
